...
(function () {
  ...
  /**
   * V1 LOCKING RETIRED (this session) — was "⚠️ TEMPORARY DEBUG SWITCH"
   * (added 2026-08-21). Resolved, not just left on: real per-category
   * locking (Phase 4's isCategoryUnlocked(), real branch below) can
   * never actually pass anymore, debug flag or not. Its rule is
   * `getCategoryProgress(prevCat).assessment?.passed` — that field is
   * only ever set by recordCategoryAssessment(), and nothing in the
   * current app calls that function. (The Missions rewrite replaced
   * V1's per-category assessment step entirely; js/lesson.js and
   * js/mastery-quiz.js only ever talk to window.LWMissions, never to
   * this module.) So the real branch isn't "strict but disabled" —
   * it's dead code that would return `false` for every category past
   * the first one, for every learner, permanently, the moment it ran.
   * Flipping this flag to `false` was on a prior fix list as the
   * "safe, low-risk" next step; it isn't — traced it and it would lock
   * the Dictionary (camera-practice.html) for everyone, including
   * learners who've genuinely completed those categories through
   * Missions, since nothing populates the field this check reads.
   *
   * The real gate now lives elsewhere and doesn't have this problem:
   * camera-practice.html's boot() (see its "MISSION LOCK SYNC" /
   * "PENDING SIGN GUARD" comments) and mission-overview.js's chip
   * rendering both gate off window.LWMissions.getMissionStatus() /
   * isSignAccessible() — real chapter/sign progress, correctly kept
   * in sync by js/lesson.js on every write. That's the actual
   * enforcement today; this module's own lock check underneath it is
   * redundant at best, actively wrong at worst.
   *
   * So: kept permanently permissive rather than "temporarily" — this
   * isn't a debug bypass anymore, it's the correct behavior until
   * isCategoryUnlocked()'s real branch is rewritten to read
   * window.LWMissions data instead of the dead V1 assessment field
   * (tracked separately — see the curriculum-numbering note on
   * getOrderedLiveCategories() below). The real branch is left in
   * place, unreachable, rather than deleted, in case that rewrite
   * wants to reuse its shape.
   */
  const V1_LOCKING_RETIRED = true;
...
async function hydrateStore() {
  ...
  function isCategoryUnlocked(level, categoryId) {
    // V1_LOCKING_RETIRED short-circuit — see its doc comment above.
    // Permanent, not a debug toggle: the real branch below can never
    // pass under the current app (nothing sets assessment.passed
    // anymore), so short-circuiting here is the correct behavior, not
    // a bypass of it. Real branch kept for reference/future reuse.
    if (V1_LOCKING_RETIRED) return true;
