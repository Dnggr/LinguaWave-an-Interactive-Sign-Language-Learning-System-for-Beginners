...
| 08-26 | Audit of Rev 8 (below) against capturesystem's `verified_batch.md`/`checklist.md` exports, per a re-run of the same task prompt — did not blindly trust Rev 8's own log entry, independently re-derived category/SIGNS/dictionary.js parity and spot-checked several `referenceUrl`s against the live lifeprint.com pages. Rev 8's `words[]`↔`SIGNS`↔`dictionary.js` coverage, dedup (I/ME, HE/SHE, GO/COME/STOP/DRINK/SLEEP/CRY/CLEAN relocations), and STAND/WALK/JUMP handshape choice all independently confirmed correct — WALK's flat-hand description in particular is lifeprint's own explicit "general WALK, not WALK-to" citation form, which is *why* it doesn't collide with STAND/JUMP's CL:V handshape (not previously stated outright in-repo). One real issue found and fixed: `HAND_ACTIONS_CATCH`'s `referenceUrl` pointed at a real lifeprint.com URL that is a content-empty stub (title only, no description/image/video) — removed the citation rather than leave a reference implying ASLU sourcing that didn't actually happen; `verified_batch.md`'s original "no dedicated ASLU page found, high confidence given how basic the sign is" caveat for CATCH (grouped with GIVE/TAKE/GET/THROW/PICK) still applies. GIVE/THROW/PULL `referenceUrl`s spot-checked and confirmed to be real, content-bearing, on-topic pages. No other changes. | `js/data.js`, `AI_MEMORY.md` |
...
| 08-26 (later, ninth session) | Content queue continuation: `body` (Unit 12, next item flagged in `PIVOT_CHECKLIST.md`'s content-queue bullet) flipped `comingSoon:false` with 16 ASLU-checked (lifeprint.com) `data.js` SIGNS entries + matching `disabled:true` `dictionary.js` placeholders, same convention as Rev 8. All 16 are simple point/touch/pat gestures — kept `detectionType:'motion'` for consistency (every word-level sign in this file routes through the motion model architecturally, not just visually-in-motion ones). FINGER has no `referenceUrl`: lifeprint.com's `finger.htm` is a content-empty stub (title only), same class of issue as the CATCH citation removed earlier — description written from general ASL knowledge instead, flagged as such rather than silently citing a page with nothing on it. Two separate `lesson.js` bug fixes, both explicitly requested: (1) live detected-sign readout no longer appends `— not "X"` text for a confident wrong guess (`updateConfidenceUI()`) — color-coding (yellow/green) and all underlying `isCorrectSign`/`showAsSuccess`/confidence-% logic unchanged, only the displayed string; (2) "Fingerspell Your Name" (`isNameDrill` branch) was showing a stale/mismatched sign video because `lessonVideoEl` was only ever touched inside the `if (signData)` branch, never reset for the drill — now explicitly paused+hidden there, **and** explicitly un-hidden again in the `if (signData)` branch so a learner visiting the drill doesn't lose the video permanently on every real sign afterward (would've been a new regression otherwise). Confirmed `fingerspell_name` itself is *not* being removed — it's a `UNITS`-only `kind:'interactive', gated:true` entry with no `CATEGORIES` entry, exactly the Rev 6 "Fingerspell-as-assessment" mechanism Joshua/Omen already confirmed (see 08-23 "Rev 6" entry above); nothing here converts it into a normal SIGNS lesson. `classifier.js`/`progress.js`/`learn.js`/`auth.js` confirmed read-only, untouched — `learn.js`'s `body` icon entries already existed from the Rev 7 pivot. **Found, not fixed (out of scope — only `fingerspell_name` was in the request):** the final `else` branch (genuinely-unwritten `comingSoon` content, not the name drill) has the identical stale-video bug — same root cause, different branch, needs its own explicit ask. Verified in Node only: `node --check` on all 3 edited `.js` files, plus a count check confirming `body`'s `words[]` (16) exactly matches its real SIGNS coverage (16) and `dictionary.js` placeholder coverage (16) — not browser-tested. | `js/data.js`, `js/engine/dictionary.js`, `js/lesson.js`, `AI_MEMORY.md`, `PIVOT_CHECKLIST.md` |
| 08-26 (later, tenth session) | Verified rather than blindly repeated the incoming task doc's Priority 1 (fingerspell stale-media fix): inspected the actual `lesson.js` `isNameDrill`/`signData` branches directly and confirmed the fix described in the ninth-session log entry above (video pause+hide in the drill branch, explicit re-show in the `signData` branch, drill-specific description/tips/image-placeholder copy) is already present and dated 2026-08-26 — same day, prior session. No code change made for Priority 1; reported the "already done" finding instead of re-implementing it. Priority 2: content queue continuation, `personal_information` (Unit 13, next item per `PIVOT_CHECKLIST.md`) flipped `comingSoon:false`. Before writing content, cross-checked all 15 `words[]` against existing `SIGNS` coverage rather than assuming all 15 needed new entries — found 9 already live under other categories (`BOY`/`GIRL` → `family`; `CHILD`/`PERSON`/`FRIEND`/`STUDENT`/`TEACHER` → `people`; `SCHOOL`/`HOME` → `places`) and reused them instead of duplicating, same convention as the school-group block's existing BOY/GIRL/HELP note. Wrote 6 new ASLU-checked (lifeprint.com) SIGNS entries for the remainder: NAME, FAMILY, BIRTHDAY, LIVE, FROM all have real, content-bearing lifeprint.com pages (`referenceUrl` set); AGE's dedicated `age.htm` is a content-empty stub (title only) — same class of issue as FINGER/CATCH in prior sessions — description written from general ASL knowledge (corroborated across several independent ASL references) instead of citing an empty page, and flagged as such. Added 6 matching `disabled:true` `dictionary.js` placeholders (parity convention, same as the `body` block). `SYSTEM_ARCHITECTURE.md` intentionally left untouched — no architecture change, and its Unit Map's detection-status column wasn't updated for `body`'s placeholders last session either, so leaving `personal_information`'s row alone matches that precedent rather than fixing it unilaterally. `js/lesson.js`, `js/engine/progress.js`, `js/learn.js` confirmed read-only and untouched — `learn.js`'s `personal_information` icon entries already existed from the Rev 7 pivot, no icon-map work needed. Verified in Node only: `node --check` on both edited `.js` files, plus a count check confirming `personal_information`'s 6 new SIGNS entries match the 6 new `dictionary.js` placeholders and the other 9 `words[]` entries each resolve via `getSign()` to their real existing category — not browser-tested. | `js/data.js`, `js/engine/dictionary.js`, `AI_MEMORY.md`, `PIVOT_CHECKLIST.md` |