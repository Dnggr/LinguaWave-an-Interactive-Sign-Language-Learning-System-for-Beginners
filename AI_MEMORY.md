...
mechanism, just a smaller Quick Check cluster size, one new recall
question format, and a new optional, localStorage-only personalization
card. `js/data.js`/`js/learn.js`/`js/engine/progress.js`/`js/auth.js`
were **not modified** — read for context, confirmed unnecessary to
touch. Full writeup: `SYSTEM_ARCHITECTURE.md` → "Rev 8";
`PIVOT_CHECKLIST.md` → "Rev 8" for the checklist/verification detail.
